...
<div class="container mx-auto mt-5">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center space-x-4">
            <h2 class="text-2xl font-semibold">Quản lý khuyến mãi</h2>
        </div>
        <div class="flex items-center space-x-4">
            ...
            <table class="min-w-full bg-white">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 tracking-wider">Mô tả</th>
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                @if($promotion->image_path)
                                    <img src="{{ asset('storage/' . $promotion->image_path) }}" alt="{{ $promotion->title }}" class="h-20 w-20 object-cover rounded">
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                {{ $promotion->is_active ? 'Đang hiển thị' : 'Đã ẩn' }}
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500 text-sm leading-5">
                                <a href="{{ route('admin.promotions.edit', $promotion) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Sửa</a>
                                <form action="{{ route('admin.promotions.destroy', $promotion) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Bạn có chắc chắn muốn xóa khuyến mãi này?')">Xóa</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
